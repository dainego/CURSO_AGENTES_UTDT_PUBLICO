...
async def main():
    ...
    USER_ID = "ahsanayaz"
    APP_NAME = "social_media_post_generator"

    session = await maybe_await(session_service.create_session(
        app_name=APP_NAME,
        user_id=USER_ID,
    ...
    # Use run_async for better compatibility with async SessionService
    ...
    session = await maybe_await(session_service.get_session(
        app_name=APP_NAME,
        user_id=USER_ID,
        session_id=SESSION_ID,
    ))
